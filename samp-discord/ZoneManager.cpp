...
void ZoneManager::AddCity(string name, float aX, float aY, float aZ, float bX, float bY, float bZ)
{
	Area coords = { aX, aY, aZ, bX, bY, bZ };

	_cityName.push_back(name);
	_cityCoords.push_back(coords);
}

void ZoneManager::AddZone(string name, float aX, float aY, float aZ, float bX, float bY, float bZ)
{
	Area coords = { aX, aY, aZ, bX, bY, bZ };

	_zoneName.push_back(name);
	_zoneCoords.push_back(coords);
}

string ZoneManager::GetCity(float x, float y, float z)
{
	string city = "Unnamed";

	int count = _cityName.size();

	for (int i = 0; i < count; i++)
	{
		string name = _cityName[i];
		Area coords = _cityCoords[i];

		if ((x >= coords.aX && x <= coords.bX) && (y >= coords.aY && y <= coords.bY) && (z >= coords.aZ && z <= coords.bZ))
		{
			city = name;
			break;
		}
...
string ZoneManager::GetZone(float x, float y, float z)
{
	...
	for (int i = 0; i < count; i++)
	{
		string name = _zoneName[i];
		Area coords = _zoneCoords[i];
